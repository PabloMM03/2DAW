...
<form  method="post" action="<?php echo $_SERVER['PHP_SELF']; 
        #Acción sobre el propio formulario, se envía a esta página?>">
<p>
...
<p>
    ...
    <?php if(!empty($_POST["checkbox"]) && in_array("Desarrollo Web en Entorno Servidor",$_POST["checkbox"])){
        echo 'checked';
    } ?>>Desarrollo Web en Entorno Servidor</input>
    
    <input name="checkbox[]" type="checkbox" id="DWC" value="Desarrollo Web en Entorno Cliente" 
    <?php if(!empty($_POST["checkbox"]) && in_array("Desarrollo Web en Entorno Cliente",$_POST["checkbox"])){
        echo 'checked';
    } ?>>Desarrollo Web en Entorno Cliente</input>
        
    <?php if(isset($_POST['enviar']) && empty($_POST['checkbox'])){ 
    #Si una vez que enviamos el formulario existen datos vacíos
    echo "Debes introducir los datos.";
    }?>
</p>
