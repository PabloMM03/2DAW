...
error_reporting(E_ALL);
 ...
 $conexion = new mysqli('localhost', 'root', '', 'campos2');
...
<?php 
  ...
  //echo '<pre>'.print_r($_POST).'</pre>';
    ...
    if(isset($_POST['delete'])){
      ...
      if(!$conexion->query('DELETE FROM campos2 WHERE id = \''.$clave.'\'')||($conexion->affected_rows == 0)){
      ...
      insertarDatos($conexion);
        ...
        $resultado = $conexion->query('SELECT * FROM campos2');
        $i= 0;
        ...
        $query = 'UPDATE campos2 SET nombre = ? WHERE id = ?';
  ...
  <div class="mainCointainer"> 
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
       <div class="addContainer">
         ...
         <input type="text" value="" placeholder="Edad" name="clientes_Edad"> 
        <input type="text" value="" placeholder="Ciudad" name="clientes_Ciudad"> <br>
        <input type="file" value="" name="clientes_Imagen"> 
       </div>
       <div class="registrosContainer">
         <?php while($datos = $resultado->fetch_array()){?> 
          <br>
<input type="submit" value="x" name="delete[<?php echo $datos['id']; ?>]"> <input type="text" value="<?php echo $datos['nombre']; ?>"  name="name[<?php echo $datos['id']; ?>]"> 
  <img src="./imagenes/<?php echo $datos['imagen']; ?>" class="imagenes1">
         <?php } ?>
